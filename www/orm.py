...
def select(sql, args, size=None):
    ...
    with (yield from __pool) as conn:
        ...
        if size:   # 如果传入的为size
            resultset = yield from cur.fetchmany(size)  # 从数据库中取出指定行数的数据
        else:
            resultset = yield from cur.fetchall()  # 否则取出所有结果集
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def findAll(cls, where=None, args=None, **kw):
        ...
        resultset = yield from select(' '.join(sql), args)   # 调用前面定义的select函数，没有指定size,因此会fetchall
